...
#may need to be tweaked although it can handle both 5 and 6
def students_per_program(lst, program):
    temp = []
    for x in lst:
        if x[4] == program:
            temp.append(x)
...
# Read data from the file and store into a data structure
...
for x in temp:
    if x[2] == '' or float(x[3]) > 100 or x[1] == '' or len(x[2]) > 10 or len(x[1]) > 10:
...
while True:
    ...
    # Get input from user
    userInput = input("Make selection: ")
